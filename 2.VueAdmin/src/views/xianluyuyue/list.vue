	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('xianluyuyue','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('xianluyuyue','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('xianluyuyue','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('xianluyuyue','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="审核" v-if="btnAuth('xianluyuyue','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('xianluyuyue','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('xianluyuyue','反馈')" type="success" @click="yuyuejieguoCrossAddOrUpdateHandler(scope.row,'cross','否','','')">反馈</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<yuyuejieguoFormModel ref="yuyuejieguoFormModelRef" @formModelChange="formModelChange"></yuyuejieguoFormModel>
	</div>
</template>
<script setup>
	...
	const approvalClick = (row) => {
		let params = {
			id:row.id,
			xianlumingcheng: row.xianlumingcheng,
			tingkaozhandian: row.tingkaozhandian,
			qidian: row.qidian,
	...
	import yuyuejieguoFormModel from '@/views/yuyuejieguo/formModel'
	const yuyuejieguoFormModelRef = ref(null)
    const yuyuejieguoCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			yuyuejieguoFormModelRef.value.init(row.id,'cross','反馈',row,'xianluyuyue',statusColumnName,tips,statusColumnValue)
		})
    }
	//查询审核状态列表
